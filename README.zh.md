...
当 Obsidian 处于后台、又冒出弹窗或通知时,Bring to Front 会自动把 Obsidian 窗口拉到最前,让你不会错过。
...
不需要任何设置——启用后就生效。只要 Obsidian 在后台、出现弹窗或通知,窗口就会自动跳到前台;窗口已在前台时则什么都不做。
...
MIT,见 [LICENSE](LICENSE)。欢迎在 [GitHub](https://github.com/rockbenben/bring-obsidian-to-front/issues) 提交问题与建议。
